...
def cutByYear_test(csv_paths, cut_list):
    for csv_path in csv_paths:
        csv = pd.read_csv(csv_path)
        for folder, detail, start, end in cut_list:
            periods= [ year for year in range(start, end+1)]
            ...
            for i in range(len(temp_csv)):
                ...
                if len(split_date) > 1:
                    ...
                    if int(split_date[0]) not in periods:
                        delete_list.append(i)
            temp_csv = temp_csv.drop(delete_list, 0)
            # print(temp_csv.columns)
            for column in temp_csv.columns:
                if column != 'Date' and column != 'KOSPI_Now':
                    temp_csv = temp_csv.drop([column], 1)
            temp_csv.to_csv('./fourth(compare)/'+ str(csv_path.split('_')[1]) + '/'+str(folder)+'/'+str(detail)+'/'+str(start)+'_'+str(end)+'.csv', index = False)

def cutByYear_whole(csv_paths, cut_list):
    for csv_path in csv_paths:
        csv = pd.read_csv(csv_path)
            ...
            for i in range(len(temp_csv)):
                ...
                if len(split_date) > 1:
                    ...
                    if int(split_date[0]) not in periods:
                        delete_list.append(i)
            temp_csv = temp_csv.drop(delete_list, 0)
            temp_csv.to_csv('./second(before_preprocessing)/whole/'+ str(csv_path.split('_')[1]) + '/' +str(start)+'_'+str(end)+'.csv', index = False)
...
# cutByYear_whole(['./주식_index_1차_전처리.csv', './주식_delta_1차_전처리.csv'],
# [
#     [2007, 2010], [2009, 2012], [2011, 2014], [2013, 2016], [2015, 2018],
#     [2007, 2012], [2010, 2015], [2013, 2018],
#     [2007, 2014], [2011, 2018]
# ])
...
# cutByYear_whole(['./주식_rate_1차_전처리.csv'],
# [
#     [2007, 2010], [2009, 2012], [2011, 2014], [2013, 2016], [2015, 2018],
#     [2007, 2012], [2010, 2015], [2013, 2018],
#     [2007, 2014], [2011, 2018]
